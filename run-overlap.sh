...
for i in ${RULES[@]}; do
	...
	if [ -f $j ]; then
	    DETECTED=0
	    rm -rf *.o infer-out
	    scan-build-9 $CLANG_OPTS clang -Wall -c $j || let DETECTED=1
	    rm -rf *.o infer-out
	    infer run $INFER_OPTS -- clang -Wall -c $j || let DETECTED=1
	    if [ $DETECTED == 1 ]; then
		echo "$j detected"
	    else
		echo "$j miss"
	    fi
	    let RULE_HIGH+=$DETECTED
